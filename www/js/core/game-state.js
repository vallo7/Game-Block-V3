/*
  ...
  --------------------------------------------------------------------
*/
import { GRID, COLOR_BANK, STONE_COLOR, ICE_COLOR, EFFECTS_LIMITS, ADS, COINS } from "../config/gameConfig.js";
...
export const Game = {
  ...
  bindEvents() {
    ...
    // FRESH START (roadmap Phase 5 — coûte des Coins au lieu d'une pub
    // récompensée) : Game.freshBoard() — vide tout le plateau
    // (contrairement à revive() qui ne touche qu'aux blocs spéciaux)
    // mais garde, elle aussi, le score et le combo en cours. SECOND
    // WIND (ci-dessus) reste sur le circuit pub récompensée, inchangé —
    // ça garde un point d'entrée gratuit et un vrai usage pour les Coins.
    this.on("freshBoardBtn", () => {
      if (this.gameOverActionLock) return;

      GameAudio.playClick();

      if (!Economy.canAfford(COINS.FRESH_START_COST)) {
      ...
      Economy.spend(COINS.FRESH_START_COST, "fresh-start");
      this.freshBoard();
    });
