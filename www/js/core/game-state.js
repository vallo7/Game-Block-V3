/*
  ...
  Phase 2 : précharge désormais un asset de grille par environnement
  (roadmap §3.2) dans `gridImages`, consommé par
  core/game-render.js#drawBoard(). Les champs `boardCache`/
  `boardCacheKey` de l'ancien rendu procédural de la grille ont été
  retirés : ils ne servent plus à rien maintenant que la grille est une
  image dessinée directement (un drawImage est déjà bon marché, plus
  besoin de cache hors-écran).
  --------------------------------------------------------------------
*/
...
export const Game = {
  ...
  init() {
    ...
    // Grille : un asset par environnement (roadmap §3.2, Phase 2). La
    // liste se déduit du registre Environments, donc un futur
    // environnement ajouté là-bas est automatiquement précargé ici
  ...
  reset() {
    ...
    // qu'au tout premier démarrage. Depuis la Phase 2, cette teinte ne
    // change plus jamais en cours de partie (le système de changement
    // de couleur de grille a été retiré) : elle reste celle-ci du
