/*
  ...
  Correctif alignement grille : chaque asset de grille peut contenir
  une marge transparente autour du contenu réellement opaque (bordure
  + cases). getContentBounds() mesure cette zone une seule fois par
  image (résultat mis en cache) et drawBoard() ne dessine QUE cette
  zone, étirée pour remplir exactement le canvas — exactement le même
  principe que BLOCK_CROP pour les blocs, mais mesuré automatiquement
  au lieu d'être calibré à la main, puisque la marge peut différer
  d'un asset de grille à l'autre.
  --------------------------------------------------------------------
*/
...
Object.assign(Game, {
  ...
  // Mesure la zone réellement opaque d'une image (rogne les marges
  // transparentes éventuelles), une seule fois par image puis mise en
  // cache par URL — voir en-tête de fichier. Échantillonnage par pas
  // de 2px : la grille est un aplat simple, une détection un peu plus
  // grossière suffit et reste quasi instantanée même sur une image de
  // plusieurs mégapixels.
  getContentBounds(img) {
    const key = img.src;

    if (!this.contentBoundsCache) this.contentBoundsCache = {};
    if (this.contentBoundsCache[key]) return this.contentBoundsCache[key];
    ...
    try {
      ...
      // Lecture des pixels bloquée (cas improbable en local) : on garde
      // l'image entière telle quelle plutôt que de casser le rendu.
      bounds = { x: 0, y: 0, w, h };
    }

    this.contentBoundsCache[key] = bounds;
    return bounds;
  },
  ...
  drawBoard() {
    ...
    // §3.2). On ne dessine que sa zone réellement opaque (voir
    // getContentBounds), étirée pour remplir exactement le canvas —
    // ainsi les cases de l'asset s'alignent pile avec les cases
    // logiques quelle que soit la marge éventuellement présente dans
    // le fichier source.
    if (gridImg && gridImg.complete && gridImg.naturalWidth) {
      const bounds = this.getContentBounds(gridImg);
      ...
      if (bounds) {
        ...
        ctx.drawImage(gridImg, 0, 0, this.canvas.width, this.canvas.height);
      }
