/*
  ...
  roadmap §3.2 — Phase 2), cases et leurs animations (spawn/place/
  validate/destroy), particules/débris, faisceaux de ligne, praise/
  combo FX, et les utilitaires géométrie / couleur qui leur sont
  propres.
  --------------------------------------------------------------------
*/
...
Object.assign(Game, {
  ...
  drawBoard() {
    ...
    // La grille est un asset image dédié par environnement depuis la
    // Phase 2 (roadmap §3.2) : un simple drawImage étiré au canvas
    // remplace l'ancien dessin procédural (64 rounded-rects + contours),
    // qui nécessitait un cache hors-écran pour rester performant sur les
    // appareils modestes. Un drawImage est déjà bon marché : le cache
    // n'est donc plus nécessaire. L'image est étirée exactement aux
    // dimensions du canvas (toujours carré) pour que chaque case de
    // l'asset s'aligne pile avec la case logique correspondante, quelle
