...
import { PRAISE, PERFECT_RUN, COMBO, COINS } from "../config/gameConfig.js";
...
Object.assign(Game, {
  ...
  validate() {
    ...
    // Trophée secret "Steady Hand" (services/achievements.js) : un
    // tracé de 6 cases d'un seul coup, quelle que soit la suite du
    // clear — vérifié ici, avant que placed ne serve à autre chose.
    Achievements.checkTrace(placed.length);
    ...
    Achievements.checkCombo(this.combo);
    Quests.trackEvent("combo", this.combo);

    // Filière 2 de déblocage du thème Ice (roadmap §3.3, 4e passe —
    // remplace l'ancienne condition d'Halloween, désormais payant en
    // Coins comme Inferno, cf. services/visualtheme.js#UNLOCKS) :
    // atteindre un combo x8 dans une partie Classic. Sans effet une fois
    // débloqué (unlockThemeByCondition() est lui-même idempotent).
    if (this.combo >= 8) {
      if (VisualTheme.unlockThemeByCondition("ice")) {
    ...
    if (count > 0) {
      ...
      // Barème dynamique (demande explicite) : chaque ligne/colonne EN
      // PLUS de la première, effacée en simultané dans CE coup (count),
      // pèse PRAISE.SIMULTANEOUS_LINE_BONUS points de "power" — bien plus
      // qu'un point de combo (streak) classique. Les deux axes comptent
      // désormais chacun pour eux-mêmes plutôt que d'être additionnés à
      // parts égales comme avant (count + combo) : un clear à 3-4 lignes
      // d'un coup grimpe fort dans le barème même sans aucun combo, et un
      // long streak de simples grimpe fort même sans aucun clear
      // multi-lignes. Le seuil de déclenchement descend à count >= 2 (au
      // lieu de 3) pour que ce nouveau poids se voie dès un double clear.
      if (count >= 2 || this.combo >= 2) {
        const power = this.combo + (count - 1) * PRAISE.SIMULTANEOUS_LINE_BONUS;
    ...
    this.turn += 1;
    Achievements.checkTurns(this.turn);

    if (emptied) {
      // Économie (roadmap Phase 5) : seule source de Coins répétable en
      // jeu, volontairement rare — cf. config/gameConfig.js#COINS.
      Economy.earn(COINS.PERFECT_CLEAR, "perfect-clear");
      Achievements.addPerfectClear();
      Quests.trackEvent("perfectClear", 1, { cumulative: true });

      // perfectClearsThisRun n'alimente plus de déblocage de thème (4e
      // passe : Halloween est désormais payant en Coins, voir plus haut
      // pour la nouvelle condition d'Ice) — il reste utile pour le
      // trophée secret "Double Perfect" (services/achievements.js).
      this.perfectClearsThisRun += 1;
      Achievements.checkPerfectClearStreak(this.perfectClearsThisRun);
  ...
  // Corrige le débordement d'écran des mots les plus longs/paliers les
  // plus grands (round de suivi : une taille fixe en CSS ne peut tenir
  // compte ni de la longueur du mot ni de la largeur réelle de l'appareil
  // en même temps). Même esprit que fitBestScoreText (game-state.js),
  // mais basé sur la largeur de fenêtre plutôt que sur un palier de
  // chiffres. `idealSize` est la taille de design (PRAISE.FONT_SIZES),
  // réduite seulement si le mot ne tiendrait pas à l'écran tel quel.
  fitPraiseText(badge, word, idealSize) {
    if (!badge) return;

    const available = (window.innerWidth || document.documentElement.clientWidth || 360) * 0.9;
    const estCharWidth = 0.62; // ratio empirique pour Baloo 2 en graisse 900
    const maxByWidth = available / Math.max(1, word.length * estCharWidth);

    badge.style.fontSize = `${Math.round(Math.max(22, Math.min(idealSize, maxByWidth)))}px`;
  },

  showPraise(level, emptied) {
    const entry = PRAISE.LEVELS.find(l => l.level === level) || PRAISE.LEVELS[0];

    Achievements.checkPraise(level);
    ...
    this.fitPraiseText(badge, entry.word, PRAISE.FONT_SIZES[level] || 32);
    void badge.offsetWidth;
  ...
  processClears() {
    ...
    Achievements.addLinesCleared(count);
    Quests.trackEvent("lines", count, { cumulative: true });
    Achievements.checkSimultaneousClear(count);
    ...
    // Faisceaux lumineux qui traversent chaque ligne détruite (round
    // "bubble pop + lumière", couplé à drawDestroyAnims côté rendu) :
    // se déclenchent désormais sur CHAQUE clear, plus seulement à
    // partir de 2 lignes simultanées. Nombre de faisceaux ET vitesse de
    // traversée sont tous les deux adaptatifs au combo en cours
    // (this.combo tient encore la valeur d'AVANT ce coup à cet instant
    // précis, cf. l'ordre des opérations dans validate() — un indicateur
    // de "chauffe" suffisant pour ce réglage). Chaque faisceau démarre
    // avec un léger décalage pour donner un effet de rafale plutôt
    // qu'un paquet qui se chevauche parfaitement à l'identique.
    const beamCount = 1 + Math.min(3, Math.floor(this.combo / 6));
    const beamDuration = Math.max(280, 620 - this.combo * 18);
    ...
    for (const key of clearedKeys) {
      ...
      // Les particules/débris génériques par case ont été retirées ici
      // (round "bubble pop") : la destruction d'un bloc se raconte
      // désormais entièrement via l'anim "pop" elle-même — grossissement,
      // anneau qui éclate, rotation — pilotée par game-render.js#drawDestroyAnims,
      // sans allouer de particules supplémentaires par case (voir aussi
      // MAX_PARTICLES/MAX_DEBRIS : un gros clear multi-lignes ne charge
      // donc plus le système de particules du tout).
  ...
  addScore(points) {
    ...
    Quests.trackEvent("score", this.score);
