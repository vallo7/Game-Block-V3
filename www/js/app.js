...
export const App = {
  ...
  async bootLoad() {
    await LoadingScreen.run(this.bootAssetList(), { minDuration: 1100, timeout: 8000 });
  },
  ...
  async goToGame() {
    ...
    try {
      ...
      await LoadingScreen.run(assets, { minDuration: 550, timeout: 5000 });
      this.showGame();
      GameEntranceFX.play();
    } finally {
  ...
  async goToMenu() {
    ...
    try {
      ...
      await LoadingScreen.run(assets, { minDuration: 450, timeout: 5000 });
      this.showMenu();
    } finally {
      this._transitioning = false;
    }
