...
export const Storage = {
  ...
  starTrophiesKey: STORAGE_KEYS.starTrophies,
  questsKey: STORAGE_KEYS.quests,
  trophyStatsKey: STORAGE_KEYS.trophyStats,
  themeUnlocksKey: STORAGE_KEYS.themeUnlocks,
  adRewardsKey: STORAGE_KEYS.adRewards,
  ...
  // ---------- Progression des trophées à étoiles (5e passe) ----------
  // Forme : { [trophyId]: { star: 0-3, at: {1:ts,2:ts,3:ts}, seen: {1:bool,2:bool,3:bool} } }
  getStarTrophies() {
    try {
      const raw = localStorage.getItem(this.starTrophiesKey);
      return raw ? JSON.parse(raw) : {};
    } catch (error) {
      return {};
    }
  },
  saveStarTrophies(data) {
    try {
      localStorage.setItem(this.starTrophiesKey, JSON.stringify(data));
    } catch (error) {}
  },
  // ---------- Quêtes quotidiennes (Phase 9) ----------
  // Forme : { resetAt, activated, lastReminderAt, quests: [{id, target,
  // reward, progress, completed, claimed, adSkippable}, ...] } — null si
  // encore jamais généré (services/quests.js#ensureFreshSet le crée).
  getQuests() {
    try {
      const raw = localStorage.getItem(this.questsKey);
  ...
  saveQuests(data) {
    try {
      localStorage.setItem(this.questsKey, JSON.stringify(data));
    } catch (error) {}
  },
  getTrophyStats() {
  ...
  // ---------- Coins gagnées via pub récompensée (services/economy.js) ----------
  // Forme : { date: "YYYY-MM-DD", count: N } — remis à zéro dès que la
  // date stockée diffère du jour courant (cf. Economy.getAdRewardState()).
  getAdRewardState() {
    try {
      const raw = localStorage.getItem(this.adRewardsKey);
      return raw ? JSON.parse(raw) : { date: "", count: 0 };
    } catch (error) {
      return { date: "", count: 0 };
    }
  },
  saveAdRewardState(state) {
    try {
      localStorage.setItem(this.adRewardsKey, JSON.stringify(state));
    } catch (error) {}
  }
};
