...
export const VisualTheme = {
  ...
  UNLOCKS: {
    ice: {
      type: "condition",
      label: "Reach a x8 combo in one Classic run"
    },
    halloween: {
      type: "coins",
      price: MARKETPLACE.THEME_PRICES.halloween,
      label: `${MARKETPLACE.THEME_PRICES.halloween} Coins`
    },
    hell: {
      type: "coins",
      price: MARKETPLACE.THEME_PRICES.hell,
      label: `${MARKETPLACE.THEME_PRICES.hell} Coins`
    }
  },
  ...
  buildSlide(theme) {
    ...
    if (!available) {
      ...
      const label = unlockInfo ? unlockInfo.label : "Locked";
    ...
    if (!available) {
      ...
      btn.innerHTML =
        '<svg viewBox="0 0 24 24" class="svg-icon"><rect x="5" y="11" width="14" height="10" rx="2"></rect><path d="M8 11V7a4 4 0 0 1 8 0v4"></path></svg>' +
        `<span>${unlockInfo ? unlockInfo.label : "Locked"}</span>`;
      btn.classList.add("is-locked");
    } else if (this.current && this.current.id === theme.id) {
      btn.innerHTML = '<svg viewBox="0 0 24 24" class="svg-icon"><path d="m5 13 4 4L19 7"></path></svg><span>Active</span>';
      btn.classList.add("is-active");
    } else {
      btn.innerHTML = '<svg viewBox="0 0 24 24" class="svg-icon" fill="currentColor" stroke="none"><path d="M8 5v14l11-7Z"></path></svg><span>Select</span>';
  ...
  updateScreenBg(theme) {
    ...
    bgEl.classList.toggle("is-locked", !this.isThemeAvailable(theme));
  },
