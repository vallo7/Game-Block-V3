...
export const VisualTheme = {
  ...
  // Adventure existe. Halloween utilise la filière 2 (mécanique, sans
  // dépendance à Adventure) ; Inferno la filière 3 (achat direct en
  // Coins, prix dans config/gameConfig.js#MARKETPLACE).
  UNLOCKS: {
    halloween: {
      type: "condition",
      label: "Perfect Clear x3 in one Classic run"
    },
    hell: {
      type: "coins",
  ...
  buildSlide(theme) {
    ...
    if (!available) {
      ...
      lock.innerHTML =
        '<svg viewBox="0 0 24 24" class="svg-icon"><rect x="5" y="11" width="14" height="10" rx="2"></rect><path d="M8 11V7a4 4 0 0 1 8 0v4"></path></svg><span>' + label + '</span>';
    ...
    if (!available) {
      const unlockInfo = this.UNLOCKS[theme.id];
      btn.textContent = unlockInfo ? unlockInfo.label : "Locked";
