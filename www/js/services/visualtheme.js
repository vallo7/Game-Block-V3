/*
  ...
  --------------------------------------------------------------------
  Thèmes visuels (fond d'écran + décor animé) et, depuis la Phase 2,
  point d'entrée du moteur d'environnements (roadmap §3) : chaque
  thème déclare l'id de son environnement ("environment"), qui pilote
  à son tour le style de bloc, l'asset de grille, le cadre du bouton
  "meilleur score" et les surcouches VFX/SFX (services/environment.js).
  Il n'y a pas d'écran de sélection d'environnement séparé : il découle
  uniquement du thème visuel actif choisi ici.
...
export const VisualTheme = {
  LIST: [
    ...
    },
    {
      ...
      // Placeholder en attendant un véritable environnement dédié
      // (roadmap §3.4) : thème encore verrouillé, non sélectionnable.
      environment: "meadow",
      startColor: null
    },
    {
  ...
  // Couleur d'accent VFX de l'environnement actif si celui-ci en
  // définit une (surcouche optionnelle), sinon la couleur de repli
  // fournie par l'appelant (typiquement la couleur de session de
  // services/theme.js) — aucune régression sur les environnements qui
  // n'ont pas encore de palette dédiée (ex. "meadow" aujourd'hui).
  ...
  applyEnvironmentStyles(env) {
    ...
    root.setProperty("--best-score-frame", `url("${env.assets.bestScoreFrame}")`);
