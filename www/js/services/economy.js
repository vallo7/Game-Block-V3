...
import { COINS } from "../config/gameConfig.js";
...
export const Economy = {
  ...
  // ---------- Section "Watch Ads for Coins" de la Marketplace ----------
  // Plafond quotidien indispensable (roadmap Phase 5, 2e passe) : sans
  // lui, une pub gratuite et rejouable à volonté annulerait entièrement
  // la rareté voulue pour l'économie en jeu (cf. config/gameConfig.js#COINS).
  todayKey() {
    return new Date().toISOString().slice(0, 10);
  },

  getAdRewardState() {
    const state = Storage.getAdRewardState();
  ...
  adRewardsRemaining() {
    return Math.max(0, COINS.AD_DAILY_LIMIT - this.getAdRewardState().count);
  },

  // Retourne true/false selon qu'une récompense a pu être accordée —
  // l'appelant est responsable de ne lancer la pub récompensée que si
  // adRewardsRemaining() > 0.
  claimAdReward() {
    const state = this.getAdRewardState();
    if (state.count >= COINS.AD_DAILY_LIMIT) return false;

    state.count += 1;
    Storage.saveAdRewardState(state);

    this.earn(COINS.AD_REWARD, "ad-reward");
    return true;
  }
};
