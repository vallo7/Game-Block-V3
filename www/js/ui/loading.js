/*
  ...
  7e passe (demande explicite) : setProgress() pilote désormais la
  barre-bloc (8 cellules, css/loading.css#.loading-blockbar) au lieu
  d'une barre de remplissage classique — chaque cellule franchie reçoit
  un petit "pop" et se colore, la prochaine cellule à venir pulse
  doucement pour montrer que ça avance même entre deux paliers.
  ...
  ("pop" de la bulle, css/loading.css).
  --------------------------------------------------------------------
*/
...
export const LoadingScreen = {
  ...
  init() {
    ...
    this.cells = Array.from(document.querySelectorAll(".loading-blockbar-cell"));
  },
  ...
  setProgress(ratio) {
    ...
    const filled = Math.round(clamped * this.cells.length);

    this.cells.forEach((cell, index) => {
      const shouldFill = index < filled;

      if (shouldFill && !cell.classList.contains("filled")) {
        cell.classList.add("filled");
        cell.classList.remove("pop");
        void cell.offsetWidth;
        cell.classList.add("pop");
      } else if (!shouldFill && cell.classList.contains("filled")) {
        cell.classList.remove("filled", "pop");
      }

      cell.classList.toggle("active", index === filled && filled < this.cells.length);
    });
  },
  ...
  show() {
    ...
    // Ne rejoue l'animation d'entrée du contenu (logo, barre-bloc) que
    // si l'écran était effectivement masqué juste avant (entrée en
    ...
    if (wasHidden) {
      ...
      this.cells.forEach(cell => cell.classList.remove("filled", "pop", "active"));
    }
  },
