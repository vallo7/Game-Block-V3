#!/usr/bin/env bash
# Vérifie que les 29 fichiers binaires attendus par le code sont bien
# présents (25 historiques + 4 ajoutés en Phase 2 pour le moteur
# d'environnements). À lancer depuis la racine de www/ :
...
if [ "$missing" -eq 0 ]; then
  echo "Tout est present (29/29)."
else
  echo "$missing fichier(s) manquant(s) sur 29."
fi
