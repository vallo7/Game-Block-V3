#!/usr/bin/env bash
# Vérifie que les 25 fichiers binaires attendus par le code sont bien
# présents. À lancer depuis la racine de www/ :
...
if [ "$missing" -eq 0 ]; then
  echo "Tout est present (25/25)."
else
  echo "$missing fichier(s) manquant(s) sur 25."
fi
